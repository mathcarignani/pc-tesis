...
// exe_path 0 c input_path input_filename output_path output_filename CoderBase
//
// (2) MASK_MODE == 3, Code with CoderPCA
// exe_path 3 c input_path input_filename output_path output_filename CoderPCA window_size err1 err2 ... errN
//
// (3) MASK_MODE == 0, Decode
// exe_path 0 d input_path input_filename output_path output_filename
//
...
//    }
    ...
    Path input_path = Path(argv[3], argv[4]);
    Path output_path = Path(argv[5], argv[6]);
    ...
    for(int i=9; i < argc; i++){ error_thresholds_vector.push_back(atoi(argv[i])); }
