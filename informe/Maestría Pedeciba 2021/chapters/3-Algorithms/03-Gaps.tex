...
\vspace{+5pt}
\Revisor{Analyzing the experimental datasets presented in Chapter~\ref{datasets}, we notice that, in general, the data gaps occur in bursts. Thus, it makes sense to consider a simple binary Markov model, such as the one defined next, sometimes referred to as the Gilbert-Elliot model \cite{markovBurstOne, markovBurstTwo}, which efficiently captures the burstiness of data gap occurrences.}
