...
def create_new_row(coder_base_row, pwlh_row):
    initial = 8
    row_length = len(coder_base_row)  # 13
    new_row = []
    ...
    for i in range(row_length):  # 1, 2, .., 12
        if i < initial or i % 2 != 0:
            ...
            hundred_percent = float(coder_base_row[i-1].replace('.', ''))
...
def parse_file(input_path, filename):
    ...
    while csv_reader.continue_reading:
        ...
        if processing_pwlh is False:
            ...
            new_row = create_new_row(coder_base_row, row)
