...
double
gsl_cheb_eval1 (const double *dCoeffsArray, int nSize, const double a, const double b, const double x)
{
  size_t i;
  double d1 = 0.0;
  double d2 = 0.0;
  ...
  for (i = nSize; i >= 1; i--)
    {
      double temp = d1;
	  d1 = y2 * d1 - d2 + dCoeffsArray[i];
      d2 = temp;
    }

  return y * d1 - d2 + 0.5 * dCoeffsArray[0];
}
